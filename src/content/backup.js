...
(() => {
  ...
  function describe(json) {
    ...
    const parts = [
      n(count.replies, 'resposta rápida', 'respostas rápidas'),
      count.scripts && n(count.scripts, 'script', 'scripts'),
      n(count.tabs, 'aba', 'abas'),
      n(count.contacts, 'contato no CRM', 'contatos no CRM'),
      count.notes && n(count.notes, 'nota', 'notas'),
      count.schedules && n(count.schedules, 'agendamento', 'agendamentos'),
      count.reminders && n(count.reminders, 'lembrete', 'lembretes'),
      count.campaigns && n(count.campaigns, 'campanha', 'campanhas'),
    ].filter(Boolean);
    const at = json && json.exportedAt ? Date.parse(json.exportedAt) : NaN;
    return { count, parts, text: parts.join(' · '), at: Number.isFinite(at) ? at : null };
  }
  ...
  async function importFlow(file) {
    ...
    const replace = choice('replace', 'Substituir tudo', 'Apaga as respostas, abas, notas, agendamentos e configurações deste navegador e deixa tudo igual ao backup.', false, true);
  ...
  ZF.backup = {
    ...
    FREQS, addPeriod, nextDue, describe, fileName, statusLines, getState, myPhone, send, sendNow, autoTick, importFlow, readBackupFile };
})();
