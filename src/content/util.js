...
(() => {
  ...
  ZF.signBlocks = (blocks, name) => {
    ...
    if (!n) return blocks;
    const prefix = `*${n}:*\n`;
    const i = blocks.findIndex((b) => b.type === 'text' && b.text && b.text.trim());
    if (i < 0 || blocks[i].text.startsWith(prefix)) return blocks;
    return blocks.map((b, j) => (j === i ? { ...b, text: prefix + b.text } : b));
  };
